...
Scans the connected sources for signals of WHO Sam is and who his family are — his
...
NEVER written silently: the router proposes it and only Sam's "yes" saves it (via the
...
def propose(conn, claude_fn=None) -> str:
    ...
    prompt = (
        "You are drafting a short IDENTITY block for Sam's assistant profile, so it can "
        ...
        "Infer Sam's own full name, and each OTHER person as a family member with a "
